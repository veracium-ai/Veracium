...
| **Author / session** | research (veracium-research); adopted by dev (v4-v19 per the cells; v20 = the round-14 fold — adopted 2026-09-03, the round-15 pin; frozen from adoption to seal) |
...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  every form outside it rejected conservatively.** The interpreter keeps a
  ...
  **Literal getattr is TABLED-ONLY (round-14): a literal attribute name does
  not establish a safe receiver** — a custom `__getattr__` makes the same name
  data on one receiver and a FACILITY on another, so safety follows from
  enforceable properties, never naming. Literal getattr is the
  DECLARED-UNCERTAINTY form — used precisely when the author does not know the
  type, default supplied — so every src probe site is tabled
  (`GETATTR_ALLOWANCES`: file, receiver expression, attribute, count, receiver
  CATEGORY, result CONSUMPTION), under ONE invariant stated above the table:
  **every tabled result is consumed as data or as the receiver's own
  behaviour — nothing tabled ever opens a connection or reaches a module
  facility.** Untabled literal sites refuse; dunder/frame names refuse
  regardless of table; the both-directions sweep verifies every row at exactly
  its count in the shipped file (a stale allowance dies on the count). **Two
  escape rules guard the table itself:** a bare `sys`/`importlib` reference
  that is not the base of an attribute access REFUSES at the escape — a
  registry-bearing module escaping analysis IS the registry escaping analysis
  (round-11's rule extended); and a literal getattr whose receiver is
  MODULE-VALUED (imports propagated through simple assignments) refuses
  regardless of the table — module-valued being the one category the census
  can establish by itself, a rebound receiver falsifying its row's category
  claim even for an unprotected module. **The DOTTED-ACCESS boundary,
  stated:** dotted access is the author's assertion of a known type, governed
  by the project's typing and tests — not a discovery form; machinery names
  via dots are already protected territory; a facility returned through an
  ordinary dotted name is object dataflow (the cells subject line, third
  application) — consistent with the completeness claim, which is over
  discovery FORMS. The round-13 closure's cost was ONE product line — the sole legitimate site rewritten to

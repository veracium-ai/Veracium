...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  `MODULE_ACCESS_FORMS` enumeration, as wide as that table and no wider, with
  ...
  door while `sys.modules` stood open — so module access is now a structured
  nine-form inventory (static import; dunder-import; import-module by
  ...
  probed mechanically with unprobed rows failing the inventory test.

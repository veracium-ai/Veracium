...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  ANNOTATIONS (round-9): a string in annotation position mentioning sqlite3 is
  parsed as an expression and held to the same positive-surface rules; a string
  that cannot parse, or is dynamically assembled with any fragment mentioning
  sqlite3, FAILS CLOSED — safety that cannot be established is not exempted,
  because this surface claims COMPLETE connection-path inventory and an
  admitted-uncounted path would contradict the design's own invariant**);

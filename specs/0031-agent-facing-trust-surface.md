...
| **0031-R1-4** | external 1 | the proposal DDL did not enforce its stated contract or support reversal | v6 §4b-ii DDL pinned + §4c-iii reversal; the reviewer's six rows R1-R6 executed from the spec's own CREATE TABLE blocks | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_proposal_cell tests/test_0031_proposal_ddl.py::test_resolution_cell -q -p no:randomly` |
...
| **0031-R1-6** | external 1 | proposal attribution and review evidence were not yet buildable | v6 §6a + the shipped acceptance corpus; the DDL executed at test time from the spec | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_proposal_cell -q -p no:randomly` |
...
| **0031-R2-5** | external 2 | resolution history was not attributable or relationally bound | v8 §4b-ii: resolver column, the FK with the PRAGMA foreign_keys=ON obligation, the hex digest CHECK; N3/N4 cells | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_resolution_cell tests/test_0031_proposal_ddl.py::test_fk_is_inert_without_the_pragma__control -q -p no:randomly` |
| **0031-R2-6** | external 2 | the requested review evidence was still absent | v8 §6a: the corpus ships in the package; DDL evidence executed from the spec | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_proposal_cell -q -p no:randomly` |
...
| **0031-R3-5** | external 3 | resolution history was not fully bound (applied_txn divergence, seq allocation) | v9: applied_txn equality from one local in one commit; seq domain in DDL, MAX(seq)+1 under the lock; the gate strengthened | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_resolution_cell -q -p no:randomly` |
| **0031-R3-6** | external 3 | the DDL and foreign-key evidence overstated enforcement (BLOB-blind CHECKs; FK proven on the wrong path) | v9: the type-affinity rule (typeof on every length/GLOB/BETWEEN predicate; N5/N8 BLOB cells); FK evidence against the pinned store opening sequence | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_proposal_cell tests/test_0031_proposal_ddl.py::test_fk_holds_under_the_pinned_store_opening_sequence tests/test_0031_proposal_ddl.py::test_todays_store_lacks_the_pragma__the_obligation_is_real -q -p no:randomly` |
...
| **0031-R4-1** | external 4 | the correction carrier was still undefined | v10 §4b: the TYPED payload (object/note/effective_date), duplicate-refusing decoder, parse authority at both sites, json_valid in DDL | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_proposal_cell -q -p no:randomly` |
| **0031-R4-2** | external 4 | the resolution-history gate did not exist (a phantom citation in the ownership table) | v10 §6a: V-RESOLUTION-LEDGER as fenced SQL, extracted at test time; the seven-history battery | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_resolution_ledger_gate -q -p no:randomly` |
| **0031-R4-3** | external 4 | the SQLite type rule did not cover affinity coercion | v10: DDL owns stored-representation domains (claims narrowed to non-coercible); the binding preamble owns types; honest-limit cells | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_coercible_text_txn_is_ddl_invisible__honest_limit tests/test_0031_proposal_ddl.py::test_numeric_resolver_coerces_to_text__honest_limit tests/test_0031_proposal_ddl.py::test_ddl_accepts_what_the_store_parse_must_refuse__honest_limit -q -p no:randomly` |
| **0031-R4-4** | external 4 | the foreign-key sentinel could not detect partial enablement | v10 §4b-ii: the connection-path INVENTORY with its classifier criterion; the central connection factory rider; the inventory-completeness sweep | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_connection_path_inventory_is_complete -q -p no:randomly` |
...
| **0031-R4-6** | external 4 | the corpus was source-contained, not execution-contained | v10 §6a + corpus/rerun.sh: the claim narrowed to source-contained, receipt versions pinned, PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 | `bash corpus/rerun.sh  # from the extracted review package; asserts import-origin inside the corpus` |
| **0031-R5-1** | external 5 | the resolution-ledger gate was six true negatives whose conjunction was not totality (accept+refuse fired none) | v11 §6a: clause 7 audits the complete state/action grammar; the NULL trap COALESCEd throughout | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_resolution_ledger_gate -q -p no:randomly` |
| **0031-R5-2** | external 5 | the connection-path inventory still missed the original `__import__` mutant said to have hardened it | v11: dynamic loading of sqlite3 prohibited (literal or unresolvable arg); the original mutant permanent | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_dynamic_sqlite_acquisition_is_refused__controls -q -p no:randomly` |
| **0031-R5-3** | external 5 | the binding-type preamble accepted strings SQLite cannot bind (a lone surrogate) | v11: the text domain DEFINED (valid UTF-8) and validated in the preamble; Phase-B ownership split stated | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_surrogate_text_raises_through_the_driver__mechanism -q -p no:randomly` |
| **0031-R6-1** | external 6 | a captured `opener = sqlite3.connect` evaded the census (the sixth rung) | v12: refusal by parenthood — an uncalled sqlite3.connect attribute refuses; a bare sqlite3 name refuses | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_captured_connect_reference_is_refused__controls -q -p no:randomly` |
| **0031-R7-1** | external 7 | `sqlite3.Connection(path)` and `sqlite3.dbapi2.connect` bypassed the census (the seventh rung) | v13: the POSITIVE SURFACE — one blessed opener, Connection annotation-only, ALLOWED_ATTRS-or-refuse, submodules refused, the runtime opener matrix | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_connection_constructors_and_submodules_refuse__controls tests/test_0031_proposal_ddl.py::test_every_runtime_opener_spelling_is_refused__matrix -q -p no:randomly` |
| **0031-R8-1** | external 8 | an evaluated annotation `sqlite3.Connection(':memory:')` acquired at def time (the eighth rung) | v14: calls processed before any exemption; the exemption admits only non-executing type-reference structure | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_evaluated_annotation_acquisition_is_refused__controls -q -p no:randomly` |
| **0031-R9-1** | external 9 | string annotations bypassed the inventory (the ninth rung) | v15: strings in annotation position parsed and the surface applied recursively; unparseable/assembled fail closed | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_string_annotations_apply_the_surface_recursively__controls -q -p no:randomly` |
| **0031-R10-1** | external 10 | the string-annotation check was gated by spelling, not acquisition capability (`_sqlite3`; the tenth rung) | v16: PROTECTED_MODULES, provenance-tracked aliases, entry unconditional, the widened fail-closed floor; TypeAlias positions (red-team) | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_acquisition_is_classified_by_provenance_not_spelling__controls tests/test_0031_proposal_ddl.py::test_type_alias_values_are_annotation_positions__controls -q -p no:randomly` |
| **0031-R11-1** | external 11 | a standard alternate module-access form (sys.modules) acquired uncounted (the eleventh rung) | v17: the enumerated, mechanically checked forms inventory with a conservative floor; the claim rescoped to the enumeration | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_capability_discovery_form tests/test_0031_proposal_ddl.py::test_capability_discovery_inventory_is_mechanically_checked -q -p no:randomly` |
| **0031-R12-1** | external 12 | namespace mappings (globals/locals/vars) were absent from the inventory (the twelfth rung) | v18: CAPABILITY_DISCOVERY_FORMS (thirteen forms incl. namespace-mappings, frame-introspection, captured-primitive, accessor-constructors); both-directions probes per form | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_capability_discovery_form tests/test_0031_proposal_ddl.py::test_capability_discovery_inventory_is_mechanically_checked -q -p no:randomly` |
| **0031-R13-1** | external 13 | the `self`/`cls` exemption assumed a runtime type Python does not guarantee (the thirteenth rung) | v19: the exemption deleted, the one project site rewritten to literal access; cls refused separately; vars(x) refused; sibling exemptions pinned by executed properties | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_receiver_names_do_not_determine_runtime_type__control tests/test_0031_proposal_ddl.py::test_the_legitimate_project_site_is_literal_and_clean__control tests/test_0031_proposal_ddl.py::test_sibling_exemptions_rest_on_documented_semantics__controls -q -p no:randomly` |
| **0031-R14-1** | external 14 | a literal attribute name did not establish a safe receiver (the fourteenth rung) | v20: GETATTR_ALLOWANCES by (file, receiver, attr) with category, consumption, count; the escape rules; swept both directions | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_identical_literal_names_differ_by_receiver__control tests/test_0031_proposal_ddl.py::test_getattr_allowances_sweep_both_directions -q -p no:randomly` |
| **0031-R15-1** | external 15 | the dotted-access boundary was syntactic rather than enforceable (the fifteenth rung) | v21 (respun at the round-16 pre-dispatch refusal): one classifier over both forms, five classes, object dataflow outside the claim for both, the partition asserted by equality and bound to the spec | `PYTHONPATH=src $PY -m pytest tests/test_0031_proposal_ddl.py::test_both_attribute_forms_share_one_classification tests/test_0031_proposal_ddl.py::test_the_one_justified_difference_is_the_non_literal_name__control tests/test_0031_proposal_ddl.py::test_shared_attribute_inventory_over_src tests/test_0031_proposal_ddl.py::test_spec_quotes_the_measured_partition_exactly -q -p no:randomly` |

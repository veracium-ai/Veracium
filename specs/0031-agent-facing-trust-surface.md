...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  **FRAME INTROSPECTION** (`FRAME_ATTRS` refused on ANY base — each carries a
  ...
  `sys._getframe` refused; two src-shaped exemptions BY
  WHAT THEY CANNOT BE: `self`/`cls` are never a module, frame, or mapping, and
  `exc.__traceback__` stays legal because the discovery step is `tb_frame`,
  which refuses; `__mro__` walks and `dir()` ruled inert by what they cannot
  do, each with a clean probe); **CAPTURED PRIMITIVES** (a discovery

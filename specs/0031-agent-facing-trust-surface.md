...
| **Retrospective** | the 0.18.1 compat release `b8a4489` (`release/0.18`, the designated Phase A rollback target) shipped 2026-09-04 under PROCESS §3b's security-hotfix carve-out, due 2026-09-11; discharged 2026-09-08 in `## Retrospective` below — the rule externally reviewed, the branch implementation's external read named as the owner's call |
...
is met, three days early, and that the rule the release implements was
externally reviewed before the release. It does NOT establish that the
implementation on `release/0.18` was reviewed by external eyes. That gap is
named here as an owner decision, not closed by this paragraph: either a
reviewer reads `b8a4489`'s diff against punch-list I4 (a short, bounded
read), or the owner rules that the twice-reviewed rule, the branch's CI and
the disclosure suffice for a patch release whose only consumers are hosts
rolling back. Nothing in the 0.18 line has moved since; the diff is the
artifact as shipped.
...
to spare; the implementation's external read outstanding and named as the
owner's call. Discharged as an obligation; the review gap stays visible above.

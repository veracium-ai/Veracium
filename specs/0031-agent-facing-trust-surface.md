...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  **FRAME INTROSPECTION** (`FRAME_ATTRS` refused on ANY base — each carries a
  ...
  same unestablishable-receiver argument, the sibling closed before asked); the
  closure's cost is ONE product line — the sole legitimate site rewritten to

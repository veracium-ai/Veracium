...
README sentence. **And non-call references refuse BY PARENTHOOD (round-6):**
any `sqlite3.connect` attribute that is not itself the func of a Call — captured,
returned, passed, assigned, any indirection — REFUSES, and a bare `sqlite3`
NAME that is not the value of an attribute access REFUSES too (the module
escaping into `getattr`, a parameter, or an alias: the NEXT spelling over,
refused before anyone plants it). Non-connect attributes (`sqlite3.Row`,
`sqlite3.IntegrityError`) stay legal everywhere, asserted by control.

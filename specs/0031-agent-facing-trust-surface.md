...
- `sqlite3.Connection` — legal only as a **NON-EXECUTING TYPE REFERENCE —
  ...
  The excluded surface is MEASURED, not gestured at: 4,487 dotted dataflow
  accesses in src, named as the honest size of what static analysis of one
  file cannot see. This is the round where the honest-limit precondition
  ...
  **refused** — a `FRAME_ATTRS` name on ANY receiver, by MEMBERSHIP never
  ...
  dataflow 4,487; dotted module-plain 248; getattr dataflow 21, the
  ...
  over the one exception. **Two escape rules guard the module classes:** a
  bare `sys`/`importlib` reference that is not the base of an attribute
  access REFUSES at the escape — a registry-bearing module escaping analysis
  IS the registry escaping analysis (round-11's rule extended); and a literal
  getattr whose receiver is MODULE-VALUED (imports propagated through simple
  assignments) refuses regardless of the inventory — module-valued being the
  one category the census can establish by itself, a rebound receiver
  falsifying its row's category claim even for an unprotected module. The round-13 closure's cost was ONE product line — the sole legitimate site rewritten to

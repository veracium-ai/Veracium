...
**Result: the gate is CLEARED. 0.64% (439 of 68,479) under lex-10 against
...
(`66d11a7`); lex-10 is the shipped detector. The aggregate now has a
...
| | lex-1 (prose history) | lex-2 | lex-4 | lex-5 | lex-6 | lex-7/8/9/10 (shipped; identical figures) |
|---|---|---|---|
...
their unambiguous inflections; lex-7 (round 2) replaces the token scan with head construction and settles at 0.64%; lex-8 adds comitative co-speakers and the third-person self-possessive, measuring identically on this corpus; lex-9 (round 3) adds or-disjunction and an artifact-vs-entity self-possessive split, again identical; lex-10 (round 4, R4-1) REMOVES that artifact carve-out — ownership is not authorship: "my own record reported a diagnosis" was read as user-authored and laundered, when the record's producer can be a doctor or a bank — and measures identical once more (439 = 0.64%; the own-artifact shapes are absent from the own-use population, which is why the carve-out bought nothing and was pure laundering risk). Recall is now

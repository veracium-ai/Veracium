...
| **Version** | **v7 — POST-ACCEPTANCE AMENDMENT: `require_source_id` (the owner's staged ruling, dev session 2026-09-12: *"Go with option C"* on dev's scope of the §F3 item "require `source_id` on third-party-derived content").** `source_id` stays optional by default (§2, rule 8, I3, I13 unchanged). A host may set `MemoryConfig.require_source_id = True`; then `remember` REFUSES, before any write, third-party-AUTHORED evidence and DECLARED third-party-derived content that carries no `source_id` (§4 rule 9, I15) — a record without one has no source identity and no revocation can ever reach it (0022 R12), which research measured on the production consumer's store (7,154 edges, every one unsourced). The 0011 §4d FLOOR is excluded on purpose: an ingest with no declared context is not a claim about the source, and only the ingest site can tell the floor from a declaration (the stored payload cannot, which is why the `doctor` check that preceded this keys on the author). The default stays False in this version; the flip to True is a later minor with a BREAKING note, on the owner's word. Surfaces: the library flag; the MCP deployment's host-set `VERACIUM_MCP_SOURCE_ID` binding (I1 kept: the served `remember` tool exposes no `source_id` argument); the CLI's `--source-id`; the refusal name `source_id_required` in the 0037 shape. Nothing is invented for existing rows (I1): unsourced records stay unrevocable by source and the doctor names them. *Prior:* **v6 — ACCEPTED 2026-08-09** — *the reviewer passed `0006`'s acceptance review (round 5, no further design round). v6 = v5 + the mechanical closure edits required for the status transition, no design change: **C1** removed the superseded "no-DDL" claims (§0b/§1/Q3) and refreshed the stale External-review row; **C2** stated the origin entropy exactly (a canonical UUIDv4 from a CSPRNG, **122 random bits** — not 128, UUIDv4 fixes 6 bits); **§11 Review closure** ledger added (PROCESS.md §4a). v5 folded F3 (v4-import absent-`origin` rejected as malformed, I14) + the origin-generation/store-lineage-identity amendments; v4 folded F1/F2 (nullable digest, one shared canonical `source_identity_digest`); v3 folded R7–R11. The `0006`↔`0014` interface freeze was reviewer-signed at round 4 and is independent of `0014`'s own acceptance.* |
...
9. **`require_source_id` (v7).** With `MemoryConfig.require_source_id` on, an ingest whose
   ...
   the source. Off by default; rule 8 and I3/I13 govern the records that exist. New
   invariant **I15**.
...
| **I15** (v7) with `require_source_id` on, a third-party-authored or DECLARED third-party-derived ingest with no `source_id` is refused before any write (no edge, no episode, no journal event), the floor is not refused, a sourced one is accepted, and the default-off store behaves as v6 | `test_require_source_id_refuses_unsourced_third_party_before_any_write`, `test_require_source_id_spares_the_floor_and_the_default_off_store`, `test_the_mcp_source_binding_is_host_set_and_the_refusal_is_named`, `test_the_served_remember_tool_exposes_no_source_id` (skips without the optional MCP SDK; `specs/skip_inventory.py`) | CI |

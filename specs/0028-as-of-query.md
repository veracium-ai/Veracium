...
fact evaluated at *that same* `now`, never a second read. *(R4-2: v10 said
...
and succeeds if the window closes within its `busy_timeout`, or fails with a bare
`database is locked` and **loses the write**; under WAL it proceeds at once and
...
measured rows). *(R4-3: "refused for the window's duration" was a THIRD
...
**Precedence:** visible successors → `SUPERSEDED`; else the edge's own
assertion → `SUCCESSOR_UNAVAILABLE`; else `HEAD`. **Total over every store
state.**
...
| **malformed reference** | A `supersedes` value that names nothing yields **the same** `SUCCESSOR_UNAVAILABLE` as a hidden one — never an exception, since a corrupt row must not fail a whole query, and never a distinct cause, since a distinguishable cause is the existence signal. The integrity diagnostic is operator-only |
...
| **concurrent writer — DEFAULT rollback-journal mode** | **A writer cannot commit while the read window remains open.** It waits up to its configured `busy_timeout`; **if the window closes first it commits afterward** (measured: 1s window, committed after 0.74s), **otherwise the commit raises SQLite's bare `database is locked` and the write is ROLLED BACK — the caller must retry** (measured: 7s window against the 5000ms default, raised after 5.02s, store unchanged). *The store's own locked-write refusal covers only the transaction's START today; the commit-time lock surfaces unwrapped — a recorded product defect, not this spec's to fix.* **The store sets no `journal_mode` (`sqlite.py` sets only `busy_timeout`, `:108`), so this is what an ordinary caller gets**; `busy_timeout` is a constructor parameter (`:91`, default 5000ms), so the boundary is the caller's own configuration against the resolution's length |
...
| **V-NO-EXISTENCE-SIGNAL** (R4-1) | a principal without scope cannot tell a HIDDEN successor from a MISSING one | the two results must be **equal as WHOLE OBJECTS**, not equal on a flag — asserted over every retired-hidden state (`corrected`, `superseded`, `absorbed_duplicate`), so a field added later reintroduces the signal by FAILING rather than passing. **Mutants: (1) v9's `omitted`/`causes` result — whole-object equality and the two-field check both fail; (2) the hand set `{"corrected"}` alone — the principal is lied to on two reachable states** | **CHECKED** `tests/test_0028_successor_lookup.py::test_the_model_holds_the_observation_table_and_both_invariants`; `::test_v9_typed_result_reintroduces_the_existence_signal`; `::test_the_hand_set_corrected_alone_lies_to_the_principal_on_two_reachable_states` |
| **V-UNAVAILABLE-NEVER-HEAD** (R4-1) | `SUCCESSOR_UNAVAILABLE` resolves to `INDETERMINATE` and never to current/head, for any principal at any `T` | the head predicate is `HEAD`-only. **Mutants: a head predicate that ignores the integrity signal; collapsing the hidden case into `HEAD`** — the second is the leak-for-a-lie trade and must fail on the table AND on this invariant | **CHECKED** `::test_the_model_holds_the_observation_table_and_both_invariants`; `::test_a_head_predicate_that_ignores_the_integrity_signal_is_refused`; `::test_collapsing_the_hidden_case_into_head_trades_a_leak_for_a_lie` |
...
**§6 holds NINETEEN invariants, in three classes stated by count rather than by the word "every":** **SIX are CHECKED TODAY** by a named runner — V-NO-EXISTENCE-SIGNAL, V-UNAVAILABLE-NEVER-HEAD and V-SUCCESSOR-REGISTRY-TOTAL against `tests/test_0028_successor_lookup.py`; V-CROSS, V-ONE-SNAPSHOT and V-ONE-CLOCK against runners v10 described but did not name, **V-ONE-CLOCK with its covered fraction stated**. **THIRTEEN are OWED at implementation**, said so per row, V-SCOPE-DIFFERENTIAL naming the design its node belongs to rather than a node that exists. *(R4 minor. v10 said "every invariant carries a named executable check" — false of sixteen rows, and the fifteen bare `—` cells made the claim unfalsifiable from the table itself. Derived by checking every cited node for a `def` in `tests/`, never by reading the column. The count is of §6's table ONLY; §6a re-lists four of these, which is why a row-start sweep of the file returns 23.)*, and **no
...
with a bare `database is locked` **and lose its write** (§5, measured). The
...
is therefore **owed at implementation, not offered here.** Round 5 carries what
is executable today: the behavioural form over `classify_as_of` as shipped
...
`tests/test_0028_successor_lookup.py`), which runs the R4-1 design over nine
store states on the shipped store through public writes. *A proof of absence over

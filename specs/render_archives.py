...
def render() -> str:
    # C7-1: the lineage the history field points at must EXIST — exact
    # correspondence between the governed domain and the sidecars, checked
    # at every render (and so at every --check in the suite and the
    # extraction).
    sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
    import package_identity as _pid
    lp = _pid.lineage_problems(ARCHIVES)
    if lp:
        raise SystemExit("LINEAGE GAPS (C7-1):\n  " + "\n  ".join(lp))
    ...
    lineage = _pid.render_lineage(ARCHIVES)

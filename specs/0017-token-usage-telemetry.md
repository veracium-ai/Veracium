...
| **External review** | ROUND 9 RETURNED 2026-08-14 (package `0017-v12-20260814T0508Z.tar.gz`): 1 blocking + 1 editorial, folded as v13; round-10 package next — architecture endorsed, R9-1 confined to close ordering |
| **Decision + date** | **ACCEPTED — external round 10, 2026-08-14** (bin (a) empty; the frozen surface is I1–I12; the two nonblocking corrections folded in the acceptance commit) |
| **Path** | full |
...
| the routing context, call context, registry, and accumulator | NEW state (§4b/§4d, external R7-1 — ALL carriers now state the SAME machine): **TWO module-global `contextvars.ContextVar`s** — the routing frame (fully-immutable `(owner, op_token)`, no user-bearing state — R4-3) and **`_active_call`** (the opaque `call_id` — R6-1); the Memory-owned ACTIVE-OPERATION REGISTRY with the **FIVE-field entry `(user_id, event, buffer, cancelled, armed_call)`**; `Memory._llm_usage`; all under ONE instance lock with the **EIGHT §4d atomic transitions** (incl. R8-1's close-deactivate); listener HANDLES with the R6-3 removal rule, `close()` unsubscription | the listener (the six-step algorithm), the terminal merge, `introspect()`, `forget()`, `close()` |
...
**The registration lifecycle is constructed (R2-3; hardened by external
R8-1):** `add_usage_listener` returns an opaque HANDLE;
`remove_usage_listener(handle)` exists and is idempotent; and
**`Memory.close()` FIRST makes its own listener INERT — a lock-guarded
`listener_active = False` transition, the EIGHTH of §4d's atomic set —
and only THEN attempts unsubscribe** (today `close()` only closes the
store — the amendment is part of this spec's implementation). The
distinction is invocation vs attribution: a failed or absent
`remove_usage_listener` cannot prevent a rogue wrapper from INVOKING the
retained callback, but post-close ATTRIBUTION is impossible by the
LINEARIZATION, not by any single check (the acceptance correction: the
step-1 read alone guarantees nothing — R9-1's interleaving proved it):
close-deactivate atomically deactivates AND cancels every active entry;
listener step 6 re-checks `listener_active` and cancellation INSIDE its
critical section; the terminal merge's cancellation check observes the
same state under the same lock — so a retained callback cannot
consume an expectation, touch a buffer, merge, audit, or emit telemetry
(external R8-1's cross-product: successful registration × absent/raising
removal × an armed in-flight operation × a post-close emission — the
store still closes, and nothing attributes). A `Memory` constructed with the INTERNAL
...
closed-`Memory` contract is the invocation-vs-attribution rule with its
linearization (external R9-C1 — the older receives-no-callbacks snapshot
wording contradicted the R8-1 boundary and is replaced):** a retained
callback MAY still be INVOKED after close (failed removal cannot prevent
a dead call, and an in-flight snapshotted emission may deliver; close
does not wait for callback quiescence — a quiescence barrier is rejected
as a deadlock surface); ATTRIBUTION after close is impossible by the §4d
close-deactivate transition's atomicity — close deactivates AND cancels
every active entry in one critical section, and both the listener's
step-6 authority and the terminal merge observe that state under the
same lock. §2c carries the control-plane rows.
...
The context primitives are **TWO MODULE-GLOBAL `contextvars.ContextVar`s**
(external R7-1's sweep: the routing frame variable below, and §4d's
`_active_call` for the call id), each shared by every `Memory` in the
process; the ROUTING variable is read by every listener
...
1. if `listener_active` is False (the §4d close-deactivate transition) →
   return, consuming NOTHING — the invocation may happen, the attribution
   cannot;
2. read the module-global routing frame; if `frame.owner is not self` →
   return, consuming nothing;
...
5. obtain the operation's event from the entry; validate the event dict
   ...
   nothing;
6. in ONE critical section under the instance lock — **RE-CHECKING
   everything steps 1–4 checked outside it (external R9-1: the step-1
   read was check-then-act; a callback paused after it could resume past
   a close)**: re-verify `listener_active`, re-resolve the entry and its
   not-cancelled state, read the calling context's `call_id` from
   `_active_call` and COMPARE it to the entry's `armed_call` (inactive
   listener, missing/cancelled entry, absent context id, no armed call, a
   MISMATCHED id, or already-consumed → drop, consuming NOTHING), then
   mark consumed and append the validated counts to the entry's buffer.
   **Steps 1–4 are early-outs; step 6 is the ONLY authority.**
...
### 4d. The registry, ONE atomic lock discipline, and erasure (external R4-3/R4-4/R6-1/R7-1)

The ACTIVE-OPERATION REGISTRY (entries: **`(user_id, event, buffer,
cancelled, armed_call)`**, where `armed_call` is `None` or `(call_id,
consumed)`) and the per-user accumulator (`Memory._llm_usage`) are
instance state under **ONE instance lock, and the EIGHT transitions are
each ATOMIC under it** (external R7-1 — the round-6 fix declared the call
transitions in §4b while this section still declared four over the old
entry shape; every normative carrier now states the same machine):

1. **operation start:** register the five-field entry
   (`armed_call = None`);
2. **arm (per provider call):** write `armed_call = (fresh call_id,
   False)` and set `_active_call` in the operation's context immediately
   before the invocation;
3. **compare-consume (listener step 6):** read the CALLING context's
   `call_id` from `_active_call`, compare to the entry's armed unconsumed
   id — absent/mismatched/consumed → drop consuming nothing; equal → mark
   consumed AND append to the buffer, one critical section;
4. **clear (per provider call, `finally`):** `armed_call = None` and
   reset `_active_call` via its token;
5. **`forget(user_id)`:** delete the user's accumulator entry AND mark
   every registry entry for that user cancelled — one critical section;
6. **terminal merge:** check the entry's cancelled flag AND (iff clear)
   merge the buffer into the accumulator — check and write never
   separated;
7. **operation exit (`finally`):** remove the registry entry;
8. **close-deactivate (external R8-1; LINEARIZED by R9-1 — the v12 form
   was itself check-then-act, the same class R4-4 fixed for `forget`):**
   `Memory.close()`, in ONE critical section under the lock and BEFORE
   attempting unsubscribe, sets `listener_active = False` AND marks every
   active registry entry cancelled — closure is forget-everything at the
   attribution level. The listener's step-1 check is a cheap early-out;
   the BINDING checks happen inside step 6, and the terminal merge's
   existing cancellation check (transition 6) then observes closure with
   no new mechanism — a callback paused past step 1, or an event buffered
   before close, both die at their respective lock-guarded checks.

Ordering is exact: a `forget` acquiring the lock before the merge wins
(the merge sees cancelled and discards); a merge holding the lock
...
accumulator, the registry, and every copied context (which holds only
opaque tokens). The accumulator remains `"scope": "instance-lifetime"`;
two instances independent; restart empty; growth bounded by served users.
...
| I12 — the registration/removal/close matrix is TOTAL (external R5-4) | `test_registration_control_plane_matrix` — every §2c control-plane cell executed: capability without `add_usage_listener`; registration raising; register-then-raise (callback retained → inert); an invalid (None) handle; unknown and duplicate handles to remove (R6-3 — any non-None returned value, `0` included, is a VALID handle); `remove_usage_listener` absent/raising; DUPLICATE `close()`; close with raising unsubscribe → the store is PROVEN closed regardless AND the listener is proven INERT first (deactivate+cancel-all in ONE critical section BEFORE unsubscribe asserted — R9-1's linearization; the R8-1 cross-product cell) | **stage-5 obligation — does NOT exist yet** |
| I8 — erasure and lifecycle hold over the ACTUAL state machine (§4b/§4d; external R7-1: this row previously tested four transitions while §4b ran seven) | `test_usage_accumulator_lifecycle` — the EIGHT atomic transitions under one lock, incl. **arm / compare-consume / clear exercised directly, with cleanup verified after success, after an operation FAILURE (the raise path clears `armed_call` and removes the entry), and after quiescence**; the R4-4 barrier race; after `forget()` + quiescence EVERY carrier inspected empty (accumulator, registry, cancel marks) and a copied context resolves to nothing; instance-lifetime label; two instances independent; restart empty; `self_check` pushes no frame; the post-close snapshot boundary exactly as §4a states it; **and the R8-1 cross-product: successful registration × absent/raising removal × an armed in-flight operation × a post-close emission — the store closes AND the retained callback cannot consume, buffer, merge, audit, or emit (mechanism: close-deactivate atomically deactivates AND cancels every active entry; step 6 is the only authority)**; **and R9-1's two barrier seams: a callback PAUSED after the step-1 check and resumed after close → step 6's lock-guarded re-check drops it; an event BUFFERED before close with the merge after → the merge's cancellation check discards; both lock orderings asserted** | **stage-5 obligation — does NOT exist yet** |
| I9 — attribution is EXACT within the stated boundary (§4b) | `test_attribution_is_exact_under_concurrency` — barrier-controlled: two users' overlapping operations; nested `answer→recall`; concurrent compiles; the shared-wrapper re-entry trace; replay OUTSIDE any armed call (drops); replay DURING the matching armed call (accepted once, producer-claimed — the §8 boundary); the same-call copied-context DUPLICATE (drops, globally consumed); **and the STALE-A-DURING-B cell (external R7-2, the exact prescription): hold `owner` and `op_token` CONSTANT, capture call A's context, clear A, arm B, emit from A's copied context → asserts the drop happens AT THE CALL-ID COMPARISON (B's expectation NOT consumed — it must not pass trivially at registry lookup), then emit B's genuine event → accepted exactly once**; a context-free event reaches only wrapper totals | **stage-5 obligation — does NOT exist yet** |
...
**Superseded by the round-10 acceptance disposition (2026-08-14): bin (a)
empty; no further design round required.** The frozen surface is the
complete §6 table I1–I12 (decision invisibility; counter/payload
completeness over the four (role, event) pairs; atomic wrapper accounting;
the carrier consent matrix; MCP omission; affirmative opt-in; the
lifecycle/close control plane with the eight-transition linearization;
exact attribution within the stated producer-claim boundary; preservation
of accepted 0015). The stage-5 obligations — every check marked
not-yet-existing — are the implementation's contract. The ten-round §9
history (the least-sure items each round, two of which the reviewer
adopted as requirements) lives in the version-history row and
`specs/reviews.py`.
...
|---|---|---|
| internal 1 (v2) | F1 the compile role's SECOND producer (maintain) · F2 pair-granular I2b · F3 the cross-user smear restated | `bebf74d` (v3) |
| ext 1 (v3) | THE REDESIGN — F1 the totals-delta cross-user prior-state side channel · F2 duck-typed opt-in · F3 the wrapper's unprotected counter · F4 self-check reach + default roles · F5 the accumulator's lifecycle + forget-erasure · F6 the three-carrier consent split | `e07be13` (v4); pre-send: the `contextvars` substitution + emit boundary (`8085a7b`, v5); archive `0017-v5-20260813T2305Z` |
| ext 2 (v5) | R2-1 ownerless broadcast · R2-2 no aggregation/commit · R2-3 self-check unconstructible + listener leak · R2-4 emission under the lock · R2-5 the forget race · R2-6 the callback protocol · R2-7 the missing matrix column · the false-CI PACKAGE BLOCKER | `d6029ff` (v6); archive `0017-v6-20260813T2341Z` |
| ext 3 (v6) | R3-1 the module-global routing variable · R3-2 recursive frame immutability · R3-3 pair validation · R3-4 the control plane · R3-5 the generation tombstone (erasure inside the erasure fix) · R3-6 the shared count predicate + float bound · R3-7 successful-operation scoping · the blocker COMPLETED | `c6f4895` (v7); archive `0017-v7-20260814T0002Z` — the pre-seal SENT-ledger ordering adopted here |
| ext 4 (v7) | R4-1 the §2c rows actually added · R4-2 the expectation token · R4-3 opaque frames + the registry (user data out of copyable contexts) · R4-4 four atomic transitions · R4-5 I8 rewritten · R4-6 self_check pushes no frame | `1fc574f` (v8); archive `0017-v8-20260814T0035Z` |
| ext 5 (v8) | R5-1 the honest producer-claim binding boundary · R5-2 registry-owned global consumption · R5-3 the six-step listener algorithm · R5-4 the executable handle predicate + I12 · R5-5 the singular close rule | `3c17863` (v9); archive `0017-v9-20260814T0339Z` — the seal gate caught the surviving `current_frame.event` carrier pre-package (`7235c99`) |
| ext 6 (v9) | R6-1 the call state fully declared (`_active_call`, `armed_call`, arm/compare-consume/clear) · R6-2 the four-cell replay family · R6-3 the executable removal rule | `cf86ba6` (v10); archive `0017-v10-20260814T0415Z` — the seal gate caught the R6-3 cell's own second column (`4c2a6c0`) |
| ext 7 (v10) | R7-1 every carrier on the same machine (two vars, five fields, seven transitions) · R7-2 the stale-A-during-B prescription into I9 | `8749a00` (v11); archive `0017-v11-20260814T0436Z` |
| ext 8 (v11) | R8-1 the close lifecycle cross-product → the eighth transition (close-deactivate) + invocation-vs-attribution | `95bfff4` (v12); archive `0017-v12-20260814T0508Z` |
| ext 9 (v12) | R9-1 the close linearized (the R8-1 fix was itself check-then-act) → deactivate+cancel-all in one section; step 6 the only authority | `6681f12` (v13); archive `0017-v13-20260814T0534Z` |
| ext 10 (v13) | **APPROVED FOR ACCEPTANCE — bin (a) EMPTY**; two nonblocking corrections (the §4a inertness claim; the §9 refresh) | THIS commit (v14): corrections + `Spec-Status: accepted` + this ledger |

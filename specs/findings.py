...
FINDINGS = [
    ...
    dict(id="M3", title="staleness cleared on same-author-class evidence",
         owner="0008", disposition="resolved", implementation="committed",
    ...
    dict(id="M4", title="`record_outcome` overwrites authorship",
         owner="0009", disposition="resolved", implementation="committed",
         release=None, advisory=None, test="0009 H1 test_outcome_authorship_is_never_overwritten",
         released_defect="0.4.5 appends a note to a summary rebuilt on every upgrade",
         current_defect="CLOSED on branch (unreleased): record_outcome now APPENDS via the CAS append_outcome_if_head — a prior judgment's author is never overwritten (accepted specs/0009)"),

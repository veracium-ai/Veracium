...
- **0037** — procedural records and the `basis` axis. **This spec consumes 0037 directly and v2 never said so (round-1 R1-5):** §1a's cause B rested, at v1–v6.0, on **V-EXTRACTOR-BLIND** keeping every `relation_kind="procedural"` relation out of the vocabulary rendered to the prompt, which is why an instruction had nowhere legal to go; since 0037 v16 (**V-EXTRACTOR-QUOTE-GATED**) a ROUTINE the user states has one — the quote-gated procedural triple — and an instruction that is not a routine still has none; `record_procedure` is the governed path this spec declines to duplicate; and §2b's drop is correct ONLY while 0037's blindness holds. **0037 is ACCEPTED** (external round 5, 2026-09-07). If V-EXTRACTOR-BLIND is ever relaxed, §2b must be re-decided — V-NO-PROCEDURAL-IN-PROMPT-VOCAB fails first and says so.
...
instruction that is not a routine still has none.* So an instruction has nowhere legal to go, and the
...
it in. The refusal above EXEMPTS a triple under a procedural relation whose
quote VERIFIED — keyed on the relation's KIND, never on the author or the text,
the same shape as the `third_party_claim` exemption. A procedural triple whose
quote does NOT verify is refused by 0037's gate first (counted
`procedural_refused`) and never reaches this rule; a DECLARATIVE triple restating
...
| **V-NO-PROCEDURAL-IN-PROMPT-VOCAB** (REWRITTEN v6.1) | cause B's premise, as it now stands: a procedural relation reaches STORAGE only through 0037 §4a-iii's quote gate — the vocabulary handed to the prompt CARRIES the registered procedural relation (0037 v16), and a triple under it is written iff its `quote` verifies verbatim against the event text on a user-authored event, else refused and counted; an instruction that is not a routine still has no relation to go to and §2b's refusal stands | the 0037 v16 nodes: `test_the_prompt_vocabulary_carries_the_procedural_relation` (the vocabulary), `test_a_procedural_emission_without_a_verifying_user_quote_is_refused_and_counted` (nothing stored without a verified quote) — `tests/test_0037_capture.py`; reuses 0037's invariant rather than restating it. *v1 asserted "no relation in the registry denotes a practice", which was FALSE on descriptions (`has_diet`) and would have failed the day 0037 shipped `follows_procedure`; v3–v6.0 reused V-EXTRACTOR-BLIND; v6.1 follows 0037 v16.* Mutant: store a procedural triple whose quote did not verify | CI |
...
```
corpus sha256: c97b5e34bd6978a8369c2dc02e6bf34a2b4eaf0d3c562536d44958ef46a37e74
```
...
oracle manifest sha256: a551c5bb53cf00e26f922a22f670999b32b7cbfa2b2176b124e279ea2b39917e

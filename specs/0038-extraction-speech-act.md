...
is REFUSED and the REFUSAL is counted.** The extraction JSON gains one field:
...
```
corpus sha256: 3d432c877cfbc86578a5e632445e59ea2485dd8062080963d72158003ea8d803
```
...
| `instructions_dropped` (report) | `ingest.py`, both return paths | `Memory.remember`'s caller; the MCP tool result that serialises the report | mechanically enumerable: the report dict's consumers are exactly today's, since the key joins an existing dict rather than creating a surface |

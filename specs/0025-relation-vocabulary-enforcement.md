...
| READ (`validate_receipt_state` + domain read) | digest × domain | the inconsistency refusal (`NULL` digest, non-`NULL` domain); pre-D2 precedence | the snapshot axis (no such input) |
| PHASE 1 (public, `graph.py`) | digest × domain | the full domain matrix; pre-D2 on sight | snapshot-absent (`raw_request_snapshot(edge)` runs unconditionally, `graph.py:126`) |

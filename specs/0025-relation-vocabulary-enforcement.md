...
| **X17** | a primary answer that yields NO USABLE `triples` is marked in the RESULT, the carrier every host has with no opt-in: `unparseable` is True for a missing key and for every non-list value (a string, a dict, `null`, a number, a boolean), and ABSENT for a good list and for a legitimately EMPTY one — both directions, because a rule that says only what must not happen is satisfiable by marking everything. A SUCCESSFUL ingest's key set is exactly X12's pinned set. **The limit:** this is the PRIMARY answer only — a failed retry stays visible as `retried > 0, recovered = 0` as `0025` §4b(1) has always had it, and the field says an answer was unusable, never WHICH shape it was; that distinction lives in the degrade record and needs a reporter | `tests/test_0039_degradation_visibility.py` — `test_a_default_host_can_tell_a_malformed_answer_from_an_empty_one` (five malformed shapes marked, empty and good clean, the successful key set pinned) and `test_the_telemetry_counter_now_counts_the_wider_class` (the flag's one consumer, through the shipped projection) |

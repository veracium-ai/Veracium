...
| **X14** | every stored `Edge.subject` matches **carries no `|`** (`ingest.subject_off_grammar` — *contains a pipe*; **the fixtures that keep storing are NAMED, not implied**: `user`, `User`, `USER`, `person:Tansy Ko`, `org:Acme Corp`, `task:release-cut`, `place:Porto, Portugal`, **`Rex`** (B07's bare name under `has_diet`), **`the landlord`**, **`org:https://acme.example`**), **EXCEPT under `third_party_claim`, exempt by relation name and checked first — a receipt record supersedes nothing, so a pipe there cannot cause the harm the rule prevents**; a pipe-carrying subject is DROPPED — written under NO subject — and counted in the PUBLIC counter `subject_refused`, **on the retry path as well as the first pass** | `test_the_extraction_prompt_subject_line_carries_no_pipe` (a BYTE check over `prompts.EXTRACT_PROMPT` — the grammar must not be re-openable to a separator reading) + `test_a_pipe_composed_subject_writes_nothing_and_counts` (a SCRIPTED provider returns `user|person:x|org:Acme`; the store is byte-identical after ingest and `subject_refused` is 1) + `test_the_subject_check_also_covers_retry_output` (the scripted provider returns an off-grammar subject on BOTH calls) + `test_a_claimant_is_exempt_only_under_third_party_claim` (*"the landlord"* stored as a claim, refused and counted under `located_at`) |

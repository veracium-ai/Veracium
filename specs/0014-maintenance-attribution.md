...
> **draft (v16, 2026-08-10) — round 12 folded (four bin-(a), classified before fixed; three
...
- **Phase 2 — STORE-LEVEL verification (`apply_supersession_plan`), REQUEST-FIRST where
  ...
  **The ALTER-path expectation is INDEPENDENTLY AUTHORED, never regenerated from the
  migration (R12-1 — `0013` §4c: "a migration may not define its own destination";
  freezing a text that was itself produced by running the migration is the measured
  §4c circularity with one extra step):** the expected path manifest is a REVIEWED
  CONSTANT, authored by hand from two independent inputs — v5's already-frozen
  constructor DDL for `supersession_operations`, and SQLite's documented
  `ALTER TABLE … ADD COLUMN` semantics (the stored `CREATE TABLE` text gains each
  column definition appended in ALTER order) — written LITERALLY into the v6 evidence
  registry at implementation and reviewed as content under this spec's acceptance.
  The derivation rule is frozen here; the literal text enters `accepted_digests` by
  review, not by execution. The migration must then PRODUCE a byte-match to the
  pre-declared expectation: a mismatch means THE MIGRATION IS WRONG — the expectation
  never moves to meet the output. NOTHING relies on the deferred
  structural-capability model.
...
- `src/veracium/portability.py` — export/import EXCLUDE the ledger (§4e). The NEW
  ...
  **`FORMAT_VERSION` bumps 4→5 (R5-4)**. Named tests: an older importer REFUSES a v5 export;
  ...
  with its totality test + three-set mutation oracle (R11-3/R12-3; incl. the repeated

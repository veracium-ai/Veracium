...
> **draft (v15, 2026-08-10) — round 11 folded (four bin-(a), classified before fixed; three
...
- **Phase 2 — STORE-LEVEL verification (`apply_supersession_plan`), REQUEST-FIRST where
  ...
  **The digest PROJECTION is durably discriminated (R9-2 — NULL `request_digest` cannot
  ...
  `MANIFESTS[v]` is a SET):** `MANIFESTS[6]` freezes BOTH exact DDL texts byte-for-byte —
  (a) the fresh v6 constructor's `sqlite_master.sql`, and (b) the v5-constructor +
  three-ALTER path's — each proven on the qualified runtime and recorded as its own
  per-path evidence entry (`v5:constructor->v6` alongside the constructor entry), per
  accepted `0013` §4e/§5c. NOTHING relies on the deferred structural-capability model.
  Named migration test (R10-3): `test_v5_to_v6_migrates_a_nonempty_receipt_table` — a v5
  store with existing receipt rows migrates; every pre-existing row reads version 1 /
  NULL response; a post-migration write reads version 2; the final schema matches the
  regenerated evidence. Replay comparison selects the projection BY THE STORED VERSION,
  never by NULL-ness of anything.
...
is frozen as a second accepted manifest per `0013` §4e (R11-1). The cost — another store

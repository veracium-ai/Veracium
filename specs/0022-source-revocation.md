...
**2 internal round(s) and 13 external round(s) with a returned VERDICT are recorded for `0022`; 15 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.
...
| internal 14 (verdict) | 2026-08-18 | 1 | SELF-FOUND — not raised by the reviewer. Quentin asked whether we are LEARNING from what the external reviewer finds, so I classified all 39 external findings by failure MECHANISM rather than symptom. They collapse into SIX classes, and five of the six were RE-FOUND after their first instance was fi… |

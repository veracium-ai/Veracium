...
| **one raw `… episodes` SQL site OUTSIDE the store package — `src/veracium/doctor.py`, DISPOSITIONED (2026-09-12)** | `veracium doctor` is a read-only linter over a snapshot COPY of the file: it reads every episode row INCLUDING retired ones, on purpose, to compare each row's `id`/`user_id` columns with its payload and to find outcome episodes naming edges that do not exist. It parses each row's payload but never reads, reports or renders `summary` (the other seat's precision note, 2026-09-12: the blob is loaded whole; the field is never touched); nothing it prints reaches a prompt, a recall, an export or the MCP surface, and it writes nothing. *Amended 2026-09-12 (the owner's word for the procedural tripwire): the doctor now reads each edge's `note` ONCE, for research's census marker screen — counts and ids only, the text itself never reported or rendered; `summary` is still never read, and extending the screen to summaries would be a further amendment here, taken deliberately.* R18's concern (a retired episode bypassing the seam into a CONSUMER) does not arise: the linter is not a consumer, and reading retired rows is the point. `test_episode_read_seam_is_sole_path` carries this site by name with the same reason; every other reader outside the store still routes through `store.episodes()` |

...
| | |
|---|---|
| **Author / session** | research (veracium-research); adopted by dev (lineage: v2/v4/v5, v7, v13, v15; v16 = the round-4 fold — adopted 2026-09-01 as the round-5 shared baseline) |
...
| the **carrier inputs** — `envelope`, `snapshot_raw`/`current_state.current_raw` (TEXT), `current_state.source_restricted` (bool), `read_token`, `view` (round-4 C2) | a missing current row ⇒ the fail-closed branch; absence never grants | unparseable text, payload/row identity disagreement, out-of-bound identity fields ⇒ refuse (never raise) | an unknown payload key is ignored; an unknown reason FENCES | a foreign payload under this row, or a foreign view riding along | **V-BIND**, **V-PARSE**, **V-CARRIER-AGREES**, **V-EXTRACT** |
...
#                   source_restricted: frozenset[str], read_token: int)` —
#                   row, standing set and sweep all evaluated inside ONE read
#                   transaction on ONE connection, so they are ONE SQLite
#                   snapshot. It REPLACES the separate `current` parameter:
...
def classify_as_of(envelope, snapshot_raw, current_state, T, now, view=None) -> Result:
    # 0. IDENTITY BINDING — now FIVE legs (snapshot, current_state, envelope,
    #    and the view), all row-sourced, all before anything else. Parse-
    ...
    scope_fields = (None if current_state.current_raw is None
                    else try_parse_and_project(current_state.current_raw,
                                               expect_id=current_state.edge_id,
    ...
    cur = scope_fields.adapted            # adapted once at rule 1, reused
    if snap is None:
    ...
    if current_state.source_restricted:
...
```
Store.current_state(user_id, edge_id) -> CurrentState(
    user_id, edge_id,                  # identity — rule 0 binds it like every leg
    current_raw: str | None,           # the edge row's serialization VERBATIM
    source_restricted: bool,           # round-4 F4: a BOOLEAN verdict, not a
                                       # digest set. ONE collective sweep proves
                                       # a boolean; returning frozenset(standing)
                                       # claimed a PER-DIGEST computation never
                                       # performed, and nothing consumes the
                                       # digests. False attribution is worse than
                                       # a narrower true claim.
...
if not store.standing_revocations(user_id):
    source_restricted = frozenset()          # THE NO-STANDING CASE, explicit:
...
**ONE CONSISTENT READ (F2), and the mechanism is EXCLUSION, not a snapshot
(round-4 F1).** Everything is computed inside an explicit `BEGIN`…`COMMIT` on
...
consecutive reads produced a mixed world. In rollback-journal mode the
guarantee is BY EXCLUSION — a SHARED lock held for the read window, writers
refused for its duration — NOT by MVCC. Saying "snapshot" here would be the
third wrong intuition in this family; what is true is *one read window, writers
excluded, all reads within it describe one world*, and it is proven by forced
interleaving at the point immediately before the scope decision, with a
discriminating pair showing the same write succeeds once the window closes.

`CurrentState` REPLACES the separate current
carrier, so there is no second current read to be stale against: row, standing
set and sweep are evaluated in ONE transaction on ONE connection — one SQLite
snapshot — and the scope projection is parsed from THAT `current_raw`. Scope,

...
def classify_as_of(envelope, snapshot_raw, current_state, T, now, view=None) -> Result:
    ...
    if view is not None:
        ...
        if cell.principal is None:
            return Result(IDENTITY_UNBOUND, held_at_K=None)   # round-9 F3: a
            # present pair must NAME its principal -- "one principal when
            # present" said ONE and v20 checked only EQUAL, which both-None
            # satisfies. Presence precedes equality. Production cannot build a
            # principal-less ScopeView (scope_read.py:307 raises), so this
            # refusal costs no legitimate path.

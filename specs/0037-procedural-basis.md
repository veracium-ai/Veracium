...
5. **Basis is DERIVED, never declared.** A verified quote derives
   `basis="stated"`: the user stated it, and the quote is the evidence.
...
9. **Render is untouched, and this is the sentence a later reader looks for
   ...
   procedure is; `describe_procedures` shows it with the `stated`
...
| **V-HOST-DECLARED** (round-1 F1; v16) through MCP the HOST-DECLARED procedural write path is the `record_procedure` tool (v16: `remember` under `direct` can also capture one through §4a-iii's quote gate, where the user's own words are the declaration): under `capability=direct` it validates the relation and writes through the library surface; under `none` it refuses with the attempted-elevation outcome, counted; MCP `remember` with any basis-shaped input writes nothing and returns the named refusal | `test_mcp_record_procedure_is_the_host_declared_procedural_write_path` | CI |
...
corpus sha256: b004e794b1a7249379fccfb8cbd59dde1dcd9b3c5f9dfbe70f851f44afc6482c

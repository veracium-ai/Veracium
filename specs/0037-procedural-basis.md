...
| MCP `record_procedure` tool — NEW (round-1 F1) | the ONLY MCP write path for procedural records: `record_procedure(summary, basis, author="user", derived_from=None, relation="follows_procedure", note=None, date=None, source_id=None)` — `author`/`derived_from` mirror MCP `remember`'s own arguments exactly; honoured under `capability=direct` only, refused under `none` with the same named outcome as an attempted elevation and counted like one; it validates the relation exactly as the library does and calls `Memory.record_procedure(author=author, context=<direct(basis) or derived(derived_from, basis)>)` | host-attested writes (0031 Phase A) | `mcp_server.py` | n/a — additive; MCP `remember` gains NOTHING and rejects any basis like the library path |
...
<name>}` (the existing tool-result shape), and an attempted elevation
...
the disagreeing state (a declarative stamp with a basis) is procedural by
floor and the NAMED outcome `kind_conflict`, never a silent inference.
...
host that removes or renames a relation leaves its DECLARATIVE-stamped
records exactly as they are today — rendered by recall as any
...
`Memory.record_procedure` (the canonical signature is in §2), which
...
context. Text the extractor reads as procedural is
...
registry says about their relation (a declarative stamp WITH a basis is a
candidate and yields `kind_conflict`, §4a). A declarative-stamped record
without a basis is NEVER a candidate, registered or not: it appears in
...
only procedural relations, and stamps `record_kind="procedural"`. A
...
- CLI: NONE in v1 (round-2 F4). `veracium remember` keeps `--author` and
  ...
  procedure command. V-ONE-PRODUCER's sweep classifies `cli.py` as a site
  ...
  validation and sole-producer test — not an option flag on a path that
  must reject basis.
...
| **V-RELATION-UNREGISTERED** (round-1 F7; NARROWED by round-2 F2) a PROCEDURAL-stamped edge whose relation is absent from the active registry is `relation_unregistered`: in neither model-context block, in `withheld` when visible, still in the store and in `introspect`; a registry that regains the relation restores it; and — the half that closes the regression — a DECLARATIVE-stamped edge whose relation is absent (a legacy custom relation, a pre-feature row) renders in recall BYTE-IDENTICALLY to the pre-feature oracle and is no describe candidate | `test_unregistered_procedural_stamp_is_named_and_unregistered_declarative_is_unchanged` | CI |
...
corpus sha256: 57668e1f694d05923805bbe2df344f037a5c32f15eb87005c1c6a7be65a474ef
...
- **Cells — GENERATED, not enumerated here (round-2 fold, research's
  ...
  `record_kind` is an AXIS (v11 — the v10 corpus was silently its
  procedural-stamped half; the declarative half collapses to two outcomes
  by basis alone, and that collapse IS the coverage that makes "a registry
  change touches no declarative record" falsifiable); each cell carries

...
shrinks its registry, disclosed in §8 and the changelog, and V-DECLARATIVE-
UNCHANGED holds because the default registry holds every relation any
...
DescribeResult:
    descriptions: list[ProcedureDescription]   # the describable records, ORDERED (below)
    withheld:     list[Withheld]               # visible-but-not-describable records, ORDERED the same way
    truncated:    bool                          # True iff `descriptions` was cut at the limit
...
One result per VISIBLE candidate: every procedural record visible to the
principal is in exactly one of the two lists, so a caller can account for
the population; `withheld` carries the id and the named outcome only.
...
| **V-RESULT-SCHEMA** (round-1 F2) every visible procedural record is in exactly ONE of `descriptions` / `withheld`; no field of either carries the `note` or any raw stored JSON; `truncated` is True iff a cut happened; the query echo is normalized | `test_describe_result_schema_accounts_for_every_visible_record` | CI |
...
- **The recognition rule:** research's frozen procedure texts supply the
  ...
  the recovery codes… is what the team does". Beside them the corpus
...
- **Least sure of (round 2):** (1) whether `relation_unregistered` — a
  ...
  renders today — is the right conservative default, or whether such
  records should keep today's declarative treatment with a warning; (2)

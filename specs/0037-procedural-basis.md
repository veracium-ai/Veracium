...
Withheld:
    edge_id: str
    outcome: Literal["relation_unregistered", "inactive", "not_yet_valid",
...
**The candidate set, stated once (v10):** the VISIBLE edges whose
relation is registered with `relation_kind="procedural"` OR is absent from
the active registry (kind unreadable — `relation_unregistered`). A
declarative record is NEVER a candidate: it appears in neither list and
never counts (the control cell in §6a) — which is why there is no
...
`relation registered ∧ active ∧ valid_now ∧
...
disclosure=<below>, basis=<from the context>)`. **Disclosure is DERIVED,
...
derived_from)` (`ingest.py:141`), so a derived-from-third-party procedure
is `use_only` exactly as a fact would be. A `record_procedure` that skipped
...
- **Cells:** author × basis × {active, inactive, future valid_from} ×
  {visible, hidden, cross-scope-visible} × {plain summary, imperative
  summary, paraphrased summary} × {registered, unregistered relation} ×
  {source unrevoked, source standing-revoked at write}, each with its
  expected `DescribeResult` membership and named outcome AND its expected
  ABSENCE from recall's two blocks. **Disclosure is NOT an axis (v10):** it
  is DERIVED by the store (`_disclosure_for` and birth-quarantine), so
  `author × disclosure` is the diagonal — the corpus carries
  `disclosure_derived` as a COMPUTED column, and a cell where disclosure
  and author disagree can exist only as a row written directly, LABELLED
  corrupted-state (0028 §4b-iii's form), never as a product cell. The
  existence-oracle cell (an unauthorised principal probing an existing
  hidden record vs a nonexistent one) is in it by name, and so is the
  DECLARATIVE CONTROL: declarative records present in the same store
  appear in neither list and do not count toward `total_describable +
  len(withheld)`.

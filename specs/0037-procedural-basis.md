...
| **External review** | round 3 sent 2026-09-06 (package `20f778c7…` @ `3a19897`, CI 34058517574) and RETURNED the same day: RETURN for revision, three blocking + four corrections, nine round-2 items closed, "the package evidence is now well formed"; round 2 sent and returned the same day (`a5161719…` @ `18d05f7`); round 1 sent 2026-09-06 (package `5d9ac247…` @ `af880b3`, CI 34049009899, both seats' legs green) and RETURNED the same day: RETURN for revision, eight blocking + seven corrections, "strong overall architecture" — folded as v8; round 2 after research's pre-seal pass |
| **Decision + date** | — |
| **Path** | full |
...
- Export (round-3 F3 — "a v2 reader ignores it" DEMONSTRATED the failure):
  ...
  malformed and the I10 rule strips both as newer than declared.
...
| **V-CORPUS-ROWS-IN-DOMAIN** (round-3 F1 + correction 4) every row of the frozen corpus validates through the declared field types — `record_kind ∈ {None, "procedural"}`, `basis ∈ {None, "stated", "observed"}`, the four-state table — today by domain check, at implementation by constructing a `Provenance` from the row; a string-only generator that emits `"declarative"` fails here | `test_every_corpus_row_is_in_the_declared_domain` | CI |
...
corpus sha256: 1bb9c9025fe8fd4a0b3f759f72697fb9a717dff61f0311ad1a31b6a126ba151e
...
- **Cells — GENERATED, not enumerated here (round-2 fold, research's
  ...
  import/restore cells of §2c are named cells; each cell carries

...
| `count` | for `volatility_defaulted`: how many triples in this `ingest_event` call were coerced; for `member_skipped`: how many list members the well-formedness filter dropped in this call, on the primary answer and the retry answer together | the offending value's text (it is model output) |
...
| 10 | a list with malformed members (`[{}, {}]`; `[valid, "junk", 7, null]` storing one fact and skipping three with `invalid: 0`) | members skipped silently | members skipped silently (the pool loop) | `member_skipped count=n`, one record per call over both loops |
...
| 12 | `instructions` of the wrong type (0038 §2c row 2) | `unparseable` placeholder | not examined on the retry (the field is not read there) | `unparseable`/`instructions_type` · — |
| 13 ⚠ | the provider CALL raises | the exception PROPAGATES — an error | swallowed, no recovery (0025 §4b(1)) | first: `_on_error` records it; retry: `retry_failed`/`provider_error` — 0025's DESIGN, not a defect |

...
evidence the asymmetry occurs, and after it lands `cause=AttributeError` going to zero is
the amendment's test rather than a hope. The commit carrying it references 0025 and this
...
| 2c | the retry returns a bare JSON array | before §2e's 0025 amendment: `AttributeError` inside the `try`, `reps = []`, ONE record with `cause=AttributeError`; after it: normalized exactly as the first extraction does — a recovery attempt, NO record unless it then fails another way. The record is the evidence the asymmetry occurred; its count going to zero is the amendment's test | V-DEGRADE-RECORDED |

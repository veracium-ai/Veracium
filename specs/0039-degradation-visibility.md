...
**The MCP tool result carries exactly ONE field this line added, `extraction_unusable`, and nothing else** (§10 Q1 asked for nothing on it, and the degrade record's content never travels; the operator counters remain stripped — V-MCP-RESULT-CARRIES-ONE, which at v17 replaced the retired V-MCP-RESULT-UNCHANGED). **No stored byte changes; no schema, no
...
**0 internal round(s) and 6 external round(s) with a returned VERDICT are recorded for `0039`; 6 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.

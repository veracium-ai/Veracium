...
| `triples` is `null`, a number or a boolean | since `0025` v16 (2026-09-10): normalized to no triples exactly like a string or a dict — ONE `primary_failed`/`shape` record, zero facts, `extraction_unusable: True`, nothing raises (*OBSOLETE at v13, the accepted row verbatim: "`TypeError: ... is not iterable` PROPAGATES; `remember` raises; `_on_error` records it with its traceback when a reporter is attached" — moved on the owner's word and disclosed in the version cells*) | SILENT before this line — a degrade (was: a propagated error) |
...
`diagnostics=None` there are zero records and the result carries `extraction_unusable: True` (`0025` v18) so the host is not left with a dict identical to a legitimate empty. The record's fields are
...
**THE RESULT SURFACE, STATED ONCE — every other mention of it in this spec points here (v18).** The ingest result's key set is exactly `0025` X12's amended set: the pre-0025 keys, the public counters, the two audit facts, the two agreement counters, and ONE outcome boolean, `extraction_unusable`, present on every path (V-RESULT-SHAPE-EXACT). The MCP `remember` tool result carries that one field and no other addition; the operator counters remain stripped (V-MCP-RESULT-CARRIES-ONE). Nothing of any degrade record's content travels on either. A host that attaches no reporter therefore learns exactly one new thing from this line, from the result: whether the primary extraction was unusable. **THE POINTER RULE (v18, research):** every other mention of the result surface in this spec is a POINTER — it names this location and the subject ("what the results carry is §2e's") and never the value: no field name, no count, no "unchanged", no "one field". The moment a pointer carries the answer it is a second copy and the sweep has two places to look. The trade is deliberate and worth paying: a reader scanning another section for what the result carries makes one hop here — one authoritative answer and one hop, rather than seven answers and no hop. A future editor who inlines the answer back into a section has rebuilt the round-6 and round-7 findings in smaller type. **No stored byte changes; no schema, no
...
the first answer is now a recorded degrade with `extraction_unusable: True`, like the string and dict beside it; making it one DID change an outcome, and the owner ruled that change on 2026-09-10 as `0025` v16 (§10 Q6, resolved). *OBSOLETE at v13, this paragraph's own text verbatim: "The three shapes that raise still raise. A `null`, numeric or boolean `triples` on the first answer is a `TypeError` today and after this spec; it is a propagated error the error hook already records, and making it a degrade would change an outcome, which this spec does not do (§10 Q6)."* Both would require carrying model output; both are recoverable
...
that is a one-word finding; **the primary battery's P6 row**, which at acceptance recorded `shape` and then let the `TypeError` propagate (OBSOLETE since v16) and now records and returns zero facts with `extraction_unusable: True` — the ordering property it instanced now rides V-RECORD-ORDER-ON-ERROR's store-failure instance; **V-NO-INLINE-SEND's control**, which
...
- [ ] every degrade path's RETURN value is as `0025` v18 states it (X12's amended exact-set test passes; the callback adds no key; every non-list primary shape yields zero facts with `extraction_unusable: True`, nothing raises)
- [ ] the MCP tool result carries exactly one field this line added, `extraction_unusable`, and the operator counters remain stripped (V-MCP-RESULT-CARRIES-ONE's exact-key-set assertion, not 0031 §4d's strip test alone)
...
**0 internal round(s) and 7 external round(s) with a returned VERDICT are recorded for `0039`; 7 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.

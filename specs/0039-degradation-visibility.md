...
| P6 | `triples` is `null`, a number or a boolean | RECORD PLUS ERROR (v11, the deliberate choice): the primary check writes exactly ONE `primary_failed`/`shape` record, then the loop raises `TypeError` as today, which `_on_error` records with its traceback and `remember` re-raises — two records in the log, degrade then error, and the operation is an error. Whether these three should stop raising is §10 Q4 | V-ANSWER-MATRIX, V-RECORD-ORDER-ON-ERROR |
...
def equivalent_runs(shape, callback):
    with monkeypatch.context() as m:
        ...
        store._now = lambda: FIXED_INSTANT                  # the injectable clock (0028 §4a)
        calls = []
...
def canonical(store):
    # every table, every row, ordered by primary key, rendered as sorted-key JSON
...
| **V-DEGRADE-RECORDED** | with a reporter attached, each recorded path writes exactly ONE record per occurrence (per call for the two counted kinds), naming the kind in §2a's closed vocabulary — on the first answer a non-list `triples` writes its ONE `primary_failed` record whether or not the loop then raises (v11); an absent volatility key, an empty `triples` list and a well-formed answer write NONE |
| **V-RECORD-ORDER-ON-ERROR** | (v11, round-2 R2-1) on the raising primary shapes (`null`, a number, a boolean) the reporter's log carries EXACTLY two records for the call, in this order: the `primary_failed`/`shape` degrade record, then `_on_error`'s error record for the `TypeError`; the operation raises; a run with `diagnostics=None` raises identically with no log | the three shapes through `Memory.remember` with a reporter whose log is read back: count == 2, order asserted by position, the exception class asserted; the mutant "error record only" and the mutant "degrade record after the error" both fail | OWED: `tests/test_0039_degradation_visibility.py::test_the_raising_primary_shapes_log_the_degrade_then_the_error` | scripted providers: one raising on the retry; one returning prose; one returning a wrong-typed `instructions`; one emitting `"banana"` for every triple; one omitting the key — the log read back | OWED: `::test_retry_failure_writes_one_record`, `::test_unparseable_writes_one_record`, `::test_drifted_volatility_writes_one_record_with_the_count`, `::test_an_absent_volatility_key_writes_nothing` |

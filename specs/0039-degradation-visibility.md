...
| **V-RESULT-UNCHANGED** | (**AMENDED 2026-09-10, after acceptance — see the version cell; the change is ADDITIVE and only on the paths that produced nothing usable**: `unparseable: True` joins the result when the primary answer yielded no usable `triples`, which is `0025` §4c's field on its own no-JSON path, not a new name. A SUCCESSFUL ingest's key set is untouched, which is where this invariant is asserted.) the ingest result's key set is exactly the set X12 pins at HEAD (0025 §4c's counters as amended 2026-09-08, 0038's `instructions_dropped`, 0023 Q4's two audit facts, 0026's two agreement counters); nothing of the degrade travels on it | INHERITED from `tests/test_0025_enforcement.py`'s X12 exact-set test | CHECKED today (the existing node) |

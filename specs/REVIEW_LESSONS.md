...
**123 external findings, raised across 23 rounds, and 11 found internally — every one classified below, exactly once.** Counts are DERIVED from `MECHANISM` in `specs/review_lessons.py`, which is checked total against the closure ledger: a finding that is not classified fails the build, and so does a class with nothing in it. Nothing in this section is a hand-kept number — R15-2 was exactly that.

| # | class | external | self-found | rounds it was raised in | recurred |
|---|---|---|---|---|---|
| 1 | **self-assertion** — A claim not produced by the thing it describes | 17 | 1 | 1, 2, 4, 5, 6, 8, 9, 12, 13, 15, 16, 23 | **yes** |
| 2 | **proxy** — The check binds a stand-in, not the property | 25 | 0 | 1, 3, 5, 7, 10, 11, 13, 15, 16, 17, 18, 19, 20, 21, 22, 23 | **yes** |
...
**The last finding that required a change to either specification was raised in round 23**, which is the most recent round to return a verdict.

...
**117 external findings, raised across 20 rounds, and 11 found internally — every one classified below, exactly once.** Counts are DERIVED from `MECHANISM` in `specs/review_lessons.py`, which is checked total against the closure ledger: a finding that is not classified fails the build, and so does a class with nothing in it. Nothing in this section is a hand-kept number — R15-2 was exactly that.
...
| 2 | **proxy** — The check binds a stand-in, not the property | 21 | 0 | 1, 3, 5, 7, 10, 11, 13, 15, 16, 17, 18, 19, 20 | **yes** |
...
**The last finding that required a change to either specification was raised in round 19.** The 2 rounds that returned a verdict since (20–21) raised packaging and process findings only — 77 of the 128 findings here are spec-scoped, and every one of them is at or before round 19. Derived from the `scope` field on each classification; nothing in this paragraph is typed.

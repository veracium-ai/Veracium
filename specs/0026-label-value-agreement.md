...
| **Internal reviewers** | research — **ten internal rounds** (GENERATED from `specs/reviews.py`, the structured ledger — round history, verdicts and closures live there, never in this static row; latest: internal round 10, 2026-08-30). eight external rounds returned so far, so external-readiness is the ledger's state to derive, not this row's to claim |
...
| markers collection | JSON array of strings, at most 8 entries |

...
| **V2** | marker ABSENCE never changes anything: empty/absent note+object → no floor, no record, byte-identical edge | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_absence_is_no_evidence` (0026-I13: the check column presented six unwritten tests as standing — CURRENT vs implementation-time is marked per row now, bound by a standing resolution test) |
| **V3** | the B02/B07 class floors at USE_ONLY — the two baseline vectors verbatim; and the floor NEVER raises (a quarantined edge stays quarantined) | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_laundered_relay_floors_use_only` |
...
| **V5** | the demotion-direction disagreement is recorded and counted with NO disposition change | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_demotion_direction_records_only` |
| **V6** | `Edge.agreement` is a structured record (markers + direction + lexicon version), None-omitted everywhere; both counters present on every result path; MCP strips; telemetry consumption DEFERRED (0026-R1-3 — no whitelisting without a 0015-conformant amendment) | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_agreement_carriers_complete`; the SHAPE is already executable now (`agreement_shape_problems`, standing) |
| **V6a** | the import boundary is MODE-SPLIT (0026-R2-2): default mode RECOMPUTES under the current lexicon (the §3d matrix — forged/malformed/foreign inputs resolve to the recomputation, counted, never consumed) and the floor runs at establishment; `restore=True` is trust-field-faithful per accepted `0005` P2 — field and disclosure verbatim, recomputation diagnostic-only; the field's export rides a FORMAT_VERSION bump per accepted `0025`, so an old reader refuses rather than silently drops | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_agreement_import_recomputes` over every matrix row, both modes; the MATRIX itself is executable data now (`import_matrix.MATRIX`, byte-bound into both projections) |
| **V7** | a marker-free store is byte-identical before and after | **IMPLEMENTATION-TIME** — graduates with `src/`; the named obligation is `test_no_markers_is_byte_identical` |

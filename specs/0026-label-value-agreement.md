...
| **Internal reviewers** | research — **12 internal rounds** (GENERATED from `specs/reviews.py`, the structured ledger — round history, verdicts and closures live there, never in this static row; latest: internal round 12, 2026-08-30). ten external rounds returned so far, so external-readiness is the ledger's state to derive, not this row's to claim |

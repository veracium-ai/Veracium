...
| **C3** *(temporary — owned for replacement by `0012`, now RULED)* reinforcement still refreshes liveness, **unchanged by this spec** | `test_reinforcement_still_advances_observed_at` — **the permission, not the prohibition.** A deletion drawn too broadly would remove liveness refresh along with the flag clearing, and no other test distinguishes them. **Whether this behaviour is right at all is `0012` — and `0012` RULED it (2026-08-08, Design 1): reinforcement should transfer NOTHING (not `observed_at`, not `confidence`), persisting the incoming edge instead. So this C3 mechanism is superseded by `0012` on `0012`'s landing; `0008`'s CONCLUSION (only `confirm()` clears the flag) is unaffected.** Marked temporary precisely so an accepted `0008` does not read as prohibiting that correction. |

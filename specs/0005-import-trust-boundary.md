...
> acceptance flip); v7 folds round 4 (one finding — the dependency moved into
> the canonical machine-readable `Spec-Requires:` line the gate actually
> parses)
...
| **Version** | v7 (2026-08-14) — four external rounds folded, findings 7/3/3/1; the per-finding history lives in `## Review closure` (PROCESS §4a) and the design deltas in the opening narrative. The operative design: the THREE-LEVER unconditional cap (author/derived_from → `THIRD_PARTY`, disclosure floored to `USE_ONLY`); the closed `restore` bool predicate; trust-field-faithful restore over the shipped canonicalization; the pinned validate-then-cap sequence with the executed 9-cell matrix; the pinned refusal warning + tampered-export regression; the mechanical call-site manifest; every §6 check an impl-commit obligation; the FORMAL 0014 §2c amendment (path-transformed-vs-persisted comparison, computed cap-equivalence classes, fixed-point path-invisibility) carried by the canonical `Spec-Requires:` line and the verbatim §7b rider, landing atomically at the acceptance flip. v7 folds round 4: R4-1 (D+F, found in the R3-3 fix) — the dependency existed only in this narrative table while the gate parses the canonical `^Spec-Requires:` header line (`check_spec_reference.py:176`); the line now exists, and the seal parses it with the gate's own regex. Nonblocking: the rider's "raw" wording corrected to "normally canonicalized but uncapped"; this row consolidated per PROCESS R3 |

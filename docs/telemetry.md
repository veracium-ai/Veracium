...
| event | fields |
|---|---|
| ingest | facts, quarantined, episodes, unparseable (1 when the provider's answer was rejected before any triple was read: no JSON object, or an `instructions` value of the wrong type), latency (ms). The result's `extraction_unusable` boolean is NOT sent. |

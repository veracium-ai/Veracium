...
def ingest_event(store, llm: Complete, user_id: str, *, event_text: str,
    ...
    # (assertable but never superseding — §4b-i, the chosen half-
    # restoration), the original survives in Edge.original_relation, and
    # the rewrite targets a registry-resident relation so it never enters
    # the vocabulary fallback below.
    n_redispositioned = 0
    for row in parsed:
    ...
    for row in parsed:
        ...
        # COHERENCE semantic state — the author rules for a re-dispositioned
        # triple (0024 §4b: the record stopped being hearsay and became the
        # user's own statement), the ORIGINAL relation otherwise (X10 is
        # scoped to the VOCABULARY fallback, which never feeds this call).
        # Established once, retained; the accepted floors below only lower.
        disclosure = _disclosure_for(
            author,
            row["relation"] if row.get("redisposition") else row["original"],
            derived_from)
        if revoked_at_birth:

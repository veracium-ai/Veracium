...
The frozen invariant surface, each check under its canonical name, plus
the two baseline-derived regression shapes (A08 movement; B02/B07 relay
preservation with the out-of-scope statement)."""
...
def _oracle(author, derived):
    """The separately-written EXACT oracle: the author rules, no
    quarantine branch (the incoherent triple's base), floors excluded
    (no revocation in these stores)."""
    if (author == EvidenceAuthor.THIRD_PARTY
            or derived == EvidenceAuthor.THIRD_PARTY):
...
def test_a08_the_movement_cell():
    ...
    subject user — post-fix it is MENTIONABLE, unquarantined, counted."""
    s = SqliteStore(":memory:")
